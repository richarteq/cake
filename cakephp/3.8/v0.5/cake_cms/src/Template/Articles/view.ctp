    ...
    <div class="table-responsive">
      ...
      <thead>
        ...
        <tbody>
            ...
            <tr>
                <td class="tableCeldUnnecessary"><?= __('Id') ?></td>
                <td><?= $this->Number->format($article->id) ?></td>
            </tr>
            <tr>
                <td class="tableCeldUnnecessary"><?= __('User') ?></td>
                <td><?= $article->has('user') ? $this->Html->link($article->user->email, ['controller' => 'Users', 'action' => 'view', $article->user->id]) : '' ?></td>
            </tr>
            <tr>
            <td class="tableCeldUnnecessary"><?= __('Title') ?></td>
                <td><?= h($article->title) ?></td>
            </tr>
            <tr>
                <td class="tableCeldUnnecessary"><?= __('Slug') ?></td>
                <td><?= h($article->slug) ?></td>
            </tr>
            <tr>
            <td class="tableCeldUnnecessary"><?= __('Published') ?></td>
                <td><?= ($article->published)?$iconCheckSquare:$iconSquare ?></td>
            </tr>
            <tr>
            <td class="tableCeldUnnecessary"><?= __('Created') ?></td>
                <td><?= h($article->created) ?></td>
            </tr>
            <tr>
                <td class="tableCeldUnnecessary"><?= __('Modified') ?></td>
                <td><?= h($article->modified) ?></td>
            </tr>

            <tr>
                <td class="actions tableCeldUnnecessary"><?= __('Actions') ?></td>
                <td>
                      ...
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal"><?= __('Cancel') . ' ' . $iconArrowBarUp ?></button></button>
...
<div class="card mb-4">
    <div class="card-header py-3">
        <h6 class="m-0 font-weight-bold text-primary"><?= __('Body') ?></h6>
    </div>
    <div class="card-body">
...
<div class="card shadow mb-4">
  <div class="card-header py-3">
    <h6 class="m-0 font-weight-bold text-primary">Related Tags</h6>
  </div>
  <div class="card-body">
    <div class="table-responsive">
      ...
      <table class="table table-hover" id="dataTable2" width="100%" cellspacing="0">
        <thead>
            <tr>
                <th scope="col" class="tableHeadUnnecessaryReally"><?= __('Id') ?></th>
                <th scope="col"><?= __('Title') ?></th>
                <th scope="col" class="tableHeadUnnecessaryReally"><?= __('Created') ?></th>
                <th scope="col" class="tableHeadUnnecessaryReally"><?= __('Modified') ?></th>
        ...
        <tbody>
            ...
            <tr>
                <td class="tableCeldUnnecessaryReally"><?= h($tags->id) ?></td>
                <td><?= h($tags->title) ?></td>
                <td class="tableCeldUnnecessaryReally"><?= h($tags->created) ?></td>
                <td class="tableCeldUnnecessaryReally"><?= h($tags->modified) ?></td>
                <td class="actions">
                    <?= $this->Html->link('<span class="labelButton">'.__('View').' </span>'.$iconEye, ['action' => 'view', $tags->id], ['class'=>'btn btn-info', 'escape'=>false]) ?>
                    <?= $this->Html->link('<span class="labelButton">'.__('Edit').' </span>'.$iconPencilSquare, ['action' => 'edit', $tags->id], ['class'=>'btn btn-warning', 'escape'=>false]) ?>
                    
                    <!-- Button trigger modal -->
                    <button type="button" class="btn btn-danger" data-toggle="modal" data-target="<?= '#deleteModalTag_'.$tags->id ?>">
                      <?= '<span class="labelButton">'.__('Delete').' </span>'.$iconTrash ?>
                    </button>
                    <!-- Modal -->
                    <div class="modal fade" id="<?= 'deleteModalTag_'.$tags->id ?>" tabindex="-1" role="dialog" aria-labelledby="<?= 'deleteModalTag_'.$tags->id.'Label' ?>" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                        <div class="modal-content">
                          <div class="modal-header">
                            <h5 class="modal-title" id="<?= 'deleteModalTag_'.$tags->id.'Label' ?>"><?= __('Delete') ?></h5>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                              <span aria-hidden="true">&times;</span>
                            </button>
                          </div>
                          <div class="modal-body">
                            <?= __('Are you sure you want to delete # ').$tags->id.'?'; ?>
                          </div>
                          <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-dismiss="modal"><?= __('Cancel') . $iconArrowBarUp ?></button></button>
                            <?= $this->Form->postLink('<span class="labelButton">'.__('Delete').' </span>'.$iconTrash, ['action' => 'delete', $tags->id], ['class'=>'btn btn-danger', 'escape'=>false]) ?>
                          </div>
                        </div>
                      </div>
                    </div>
